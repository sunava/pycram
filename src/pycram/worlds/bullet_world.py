...
class BulletWorld(World):
    ...
    def __init__(self, mode: WorldMode = WorldMode.DIRECT, is_prospection_world: bool = False, sim_frequency=240):
        ...
        if not is_prospection_world:
            _ = Object("floor", ObjectType.ENVIRONMENT, "plane" + self.extension, world=self, pose=Pose([0, 0, -0.14]))
    ...
    def _load_object_and_get_id(self, path: str, pose: Pose) -> int:
        ...
        return p.loadURDF(path, basePosition=pose.position_as_list(), baseOrientation=pose.orientation_as_list(),
    ...
    def add_constraint(self, constraint: Constraint) -> int:
        ...
        constraint_id = p.createConstraint(constraint.parent_object_id, constraint.parent_link_id,
                                           constraint.child_object_id, constraint.child_link_id, constraint.type.value,
                                           constraint.axis_as_list, constraint.position_wrt_parent_as_list,
                                           constraint.position_wrt_child_as_list,
                                           constraint.orientation_wrt_parent_as_list,
                                           constraint.orientation_wrt_child_as_list, physicsClientId=self.id)
    ...
    def get_object_joint_names(self, obj: Object) -> List[str]:
        return [p.getJointInfo(obj.id, i, physicsClientId=self.id)[1].decode('utf-8') for i in
                range(self.get_object_number_of_joints(obj))]
    ...
    def get_object_link_names(self, obj: Object) -> List[str]:
        ...
        return [p.getJointInfo(obj.id, i, physicsClientId=self.id)[12].decode('utf-8') for i in range(num_links)]
    ...
    def add_vis_axis(self, pose: Pose, length: Optional[float] = 0.2) -> None:
        """
        ...
        multibody = MultiBody(base_visual_shape_index=-1, base_pose=pose_in_map,
                              link_visual_shape_indices=[vis_x, vis_y, vis_z], link_poses=[Pose(), Pose(), Pose()],
                              link_masses=[1.0, 1.0, 1.0], link_inertial_frame_poses=[Pose(), Pose(), Pose()],
                              link_parent_indices=[0, 0, 0],
    ...
    def ray_test_batch(self, from_positions: List[List[float]], to_positions: List[List[float]],
        ...
        return p.rayTestBatch(from_positions, to_positions, numThreads=num_threads, physicsClientId=self.id)
    ...
    def create_visual_shape(self, visual_shape: VisualShape) -> int:
        return p.createVisualShape(visual_shape.visual_geometry_type.value,
                                   ...
                                   visualFramePosition=visual_shape.visual_frame_position, physicsClientId=self.id,
                                   **visual_shape.shape_data())
    ...
    def create_multi_body(self, multi_body: MultiBody) -> int:
        return p.createMultiBody(baseVisualShapeIndex=-multi_body.base_visual_shape_index,
                                 ...
                                 linkInertialFramePositions=[pose.position_as_list() for pose in
                                                             multi_body.link_inertial_frame_poses],
                                 linkInertialFrameOrientations=[pose.orientation_as_list() for pose in
                                                                multi_body.link_inertial_frame_poses],
    ...
    def get_images_for_target(self, target_pose: Pose, cam_pose: Pose, size: Optional[int] = 256) -> List[np.ndarray]:
        ...
        return list(p.getCameraImage(size, size, view_matrix, projection_matrix, physicsClientId=self.id))[2:5]
...
class Gui(threading.Thread):
    ...
    def run(self):
        ...
        if self.mode == WorldMode.DIRECT:
            ...
            sphere_uid = p.createMultiBody(baseMass=0.0, baseInertialFramePosition=[0, 0, 0],
                                           baseVisualShapeIndex=sphere_visual_id, basePosition=camera_target_position,
                                           physicsClientId=self.world.id)
            ...
            while p.isConnected(self.world.id):
                ...
                width, height, dist = (
                p.getDebugVisualizerCamera()[0], p.getDebugVisualizerCamera()[1], p.getDebugVisualizerCamera()[10])
                ...
                if mouse:
                    for m in mouse:
                        ...
                        if mouse_state[0] == 3:
                            speed_x = abs(old_mouse_x - mouse_x) if (
                                                                        abs(old_mouse_x - mouse_x)) < max_speed else max_speed
                            speed_y = abs(old_mouse_y - mouse_y) if (
                                                                        abs(old_mouse_y - mouse_y)) < max_speed else max_speed
